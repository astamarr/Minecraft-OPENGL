...
public:
	...
	inline NYCube* getCube(int x, int y, int z)
	{
		...
		if (z < 0)
			z = 0;
		if (x >= MAT_SIZE * NYChunk::CHUNK_SIZE)
			x = (MAT_SIZE * NYChunk::CHUNK_SIZE) - 1;
		if (y >= MAT_SIZE * NYChunk::CHUNK_SIZE)
			y = (MAT_SIZE * NYChunk::CHUNK_SIZE) - 1;
		if (z >= MAT_HEIGHT * NYChunk::CHUNK_SIZE)
			z = (MAT_HEIGHT * NYChunk::CHUNK_SIZE) - 1;
	...
	*/
	...
	void load_pile(int x, int y, int height, bool onlyIfZero = true)
	{
		...
		if (height - 1 > 0 && height > 40) {
			getCube(x, y, height - 1)->_Draw = true;
			getCube(x, y, height - 1)->_Type = CUBE_NEIGE;
		}
		else if (height - 1 > 0 && height < 19) {
			getCube(x, y, height - 1)->_Draw = true;
			getCube(x, y, height - 1)->_Type = CUBE_EAU;
		}
		else if (height - 1 > 0 && height >= 19 && height < 20) {
			getCube(x, y, height - 1)->_Draw = true;
			getCube(x, y, height - 1)->_Type = CUBE_SABLE;
		}
		else if (height - 1 > 0) {
			getCube(x, y, height - 1)->_Draw = true;
		...
		int x4, int y4, int prof, int profMax = -1)
	{
		...
		if ((x2 - x1) > 1) {
			heighta += (2 % largeurRandom) - (largeurRandom / 2);
	...
	void init_world(int profmax = -1)
	{
		...
		//On attaque le perlin avec un choix de Seed en fonction de l'heure.
		time_t t = time(0);
		struct tm* now = localtime(&t);
		int RandSeed = now->tm_hour + now->tm_min + now->tm_sec;

		PerlinNoise* Perlin = new PerlinNoise(RandSeed);
		double Precision = 80.1456165418; // plutôt smooth
										  // slope pour l'extrapol perlin/hauteur
		double slope = 1.0 * (64 - 1) / (1 - 0);

		for (int x = 0; x < MAT_SIZE_CUBES; x++) {

			for (int y = 0; y < MAT_SIZE_CUBES; y++) {
				double value = Perlin->noise(x / Precision, y / Precision, 0); // perlin 2D
				double output = 1 + slope * (value - 0);
				output = output;
				if (output < 18)
				{
					output = 18;
				}
				_MatriceHeights[x][y] = output;
				load_pile(x, y, _MatriceHeights[x][y], false);
			}
		}
	...
	NYAxis getMinCol(NYVert3Df pos, NYVert3Df dir, float width, float height, float& valueColMin, bool oneShot)
	{
		...
		if (getCube(xPrev, yPrev, zPrev)->isSolid() || getCube(xPrev, yPrev, zNext)->isSolid() || getCube(xPrev, yNext, zPrev)->isSolid() || getCube(xPrev, yNext, zNext)->isSolid()) {
			//On verif que resoudre cette collision est utile
			if (!(getCube(xPrev + 1, yPrev, zPrev)->isSolid() || getCube(xPrev + 1, yPrev, zNext)->isSolid() || getCube(xPrev + 1, yNext, zPrev)->isSolid() || getCube(xPrev + 1, yNext, zNext)->isSolid()) || !oneShot) {
		...
		if (getCube(xNext, yPrev, zPrev)->isSolid() || getCube(xNext, yPrev, zNext)->isSolid() || getCube(xNext, yNext, zPrev)->isSolid() || getCube(xNext, yNext, zNext)->isSolid()) {
			//On verif que resoudre cette collision est utile
			if (!(getCube(xNext - 1, yPrev, zPrev)->isSolid() || getCube(xNext - 1, yPrev, zNext)->isSolid() || getCube(xNext - 1, yNext, zPrev)->isSolid() || getCube(xNext - 1, yNext, zNext)->isSolid()) || !oneShot) {
		...
		if (getCube(xPrev, yNext, zPrev)->isSolid() || getCube(xPrev, yNext, zNext)->isSolid() || getCube(xNext, yNext, zPrev)->isSolid() || getCube(xNext, yNext, zNext)->isSolid()) {
			//On verif que resoudre cette collision est utile
			if (!(getCube(xPrev, yNext - 1, zPrev)->isSolid() || getCube(xPrev, yNext - 1, zNext)->isSolid() || getCube(xNext, yNext - 1, zPrev)->isSolid() || getCube(xNext, yNext - 1, zNext)->isSolid()) || !oneShot) {
		...
		if (getCube(xPrev, yPrev, zPrev)->isSolid() || getCube(xPrev, yPrev, zNext)->isSolid() || getCube(xNext, yPrev, zPrev)->isSolid() || getCube(xNext, yPrev, zNext)->isSolid()) {
			//On verif que resoudre cette collision est utile
			if (!(getCube(xPrev, yPrev + 1, zPrev)->isSolid() || getCube(xPrev, yPrev + 1, zNext)->isSolid() || getCube(xNext, yPrev + 1, zPrev)->isSolid() || getCube(xNext, yPrev + 1, zNext)->isSolid()) || !oneShot) {
		...
		if (getCube(xPrev, yPrev, zNext)->isSolid() || getCube(xPrev, yNext, zNext)->isSolid() || getCube(xNext, yPrev, zNext)->isSolid() || getCube(xNext, yNext, zNext)->isSolid()) {
			//On verif que resoudre cette collision est utile
			if (!(getCube(xPrev, yPrev, zNext - 1)->isSolid() || getCube(xPrev, yNext, zNext - 1)->isSolid() || getCube(xNext, yPrev, zNext - 1)->isSolid() || getCube(xNext, yNext, zNext - 1)->isSolid()) || !oneShot) {
		...
		if (getCube(xPrev, yPrev, zPrev)->isSolid() || getCube(xPrev, yNext, zPrev)->isSolid() || getCube(xNext, yPrev, zPrev)->isSolid() || getCube(xNext, yNext, zPrev)->isSolid()) {
			//On verif que resoudre cette collision est utile
			if (!(getCube(xPrev, yPrev, zPrev + 1)->isSolid() || getCube(xPrev, yNext, zPrev + 1)->isSolid() || getCube(xNext, yPrev, zPrev + 1)->isSolid() || getCube(xNext, yNext, zPrev + 1)->isSolid()) || !oneShot) {
	...
	void render_world_vbo(void)
	{
		for (int x = 0; x < MAT_SIZE; x++)
			for (int y = 0; y < MAT_SIZE; y++)
				for (int z = 0; z < MAT_HEIGHT; z++) {
					glPushMatrix();
					glTranslatef((float)(x * NYChunk::CHUNK_SIZE * NYCube::CUBE_SIZE),
						(float)(y * NYChunk::CHUNK_SIZE * NYCube::CUBE_SIZE),
						(float)(z * NYChunk::CHUNK_SIZE * NYCube::CUBE_SIZE));
		...
		for (int i = 0; i < _ExplodedCubes.size(); i++) {
			...
			case CUBE_HERBE:
				...
				break;
			case CUBE_EAU:
				glColor3f(28.0f / 255.0f, 163.0f / 255.0f, 236.0f / 255.0f);
				break;
			case CUBE_NEIGE:
				glColor3f(1.0, 1.0, 1.0);
				break;
			case CUBE_SABLE:
				glColor3f(239.0f / 255.0f, 221.0f / 255.0f, 111.0f / 255.0f);
				break;
			}
