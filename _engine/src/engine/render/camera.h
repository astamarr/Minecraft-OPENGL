...
public:
	...
	NYFloatMatrix _ViewMatrix; ///< Pour les shaders, pour repasser de la modelview a la model
	...
	*/
	...
	{
		// Direction
		NYVert3Df dirVector = _LookAt - _Position;
		dirVector.normalize();
		_Direction = dirVector;

		//Right
		//NYVert3Df up = NYVert3Df(0, 0, 1);
		_UpVec = _UpRef;
		NYVert3Df rightVector = _Direction.vecProd(_UpRef);
		_NormVec = rightVector;
	...
	*/
	void rotate(float angle)
	{
		...
		_LookAt -= _Position;
		_LookAt.rotate(NYVert3Df(0, 0, 1), angle);
		_LookAt += _Position;
	...
	*/
	...
	{
		_Position -= _LookAt;
		_Position.rotate(NYVert3Df(0, 0, 1), angle);
		_Position += _LookAt;
